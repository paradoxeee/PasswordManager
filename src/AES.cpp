...
const std::string EasyAES::encrypt(const std::string &text, const std::string &key)
{
  ...
  // Filling padding
  for (unsigned int i = text.size(); i < paddedLen; ++i)
  {
    plain[i] = paddingLength;
  }
  ...
  std::memcpy(ckey, key.c_str(), key.size());
...
const std::string EasyAES::decrypt(const std::string &cipher, const std::string &key)
{
  ...
  {
    return "Invalid padding length"; // wrong padding or key, return empty string
    // throw std::invalid_argument("Invalid padding length");
  }
  for (unsigned int i = cipher.size() - paddingLength; i < dataLen; ++i)
  {
    if (decipher[i] != paddingLength)
    {
      return "Invalid padding"; // wrong padding or key, return empty string
      // throw std::invalid_argument("Invalid padding");
    }
  }

  std::string decipher_str(reinterpret_cast<char const *>(decipher), dataLen - paddingLength);
